...
これが最大の課題になるでしょう。多くのサービスは「入力データを学習に利用しない」という設定を提供していますが、「入力データは外部ネットワークに送信されない」ではありません。
...
このような秘匿案件では、クラウド型 AI の利用が承認されないケースは珍しくないです。AI を利用すれば短時間で終わるタスクを長々と時間をかける手作業する、AI 開発を味わってしまった現在の開発者には苦行です。
...
- 何度使っても利用制限はなく、追加コストはかからない（PC パーツや電気代を除く）
- 自身が選択したモデルやバージョンを利用し続けられる
- 量子化レベルやコンテキスト長など、細かなパラメータを自身で制御できる

### ローカル LLM の性能

少し前までのローカル LLM は、正直なところ精度は低かったです。しかし、ここ最近公開されたオープンウェイトモデルの性能は向上しました。現在の主要クラウド型と比べると目を落りしますが、コード生成タスクはできるようになりました。LLM のベンチマーク比較サイト [^artificialanalysis] [^benchlm] を参照すると、現在のローカル LLM のスコアは、クラウド LLM の前世代に近いレベルです。
