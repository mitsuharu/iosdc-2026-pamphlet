...
モデルのパラメータが 32B（320 億）の場合、単純計算で 1 パラメータは 16 bit（2 byte）なので、約 64 GB のメモリが必要です。家庭用 PC には大きな負担です。そこで有効な技術が量子化です。モデルの重みを低ビット化することで、メモリ使用量を抑えられます。一般的な Q4（４ビット量子化）は、品質劣化をできるだけ抑えつつ、メモリを大きく削減できます。理論上は 1/4 ですが、実際は KV キャッシュや実行時バッファなどがあるので、64 GB のモデルは約 21 GB になります。
