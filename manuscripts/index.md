...
モデルのパラメータが 32B（320 億）の場合、単純計算では 1 パラメータあたり 16 bit（2 byte）なので約 64 GB のメモリが必要です。家庭用 PC には大きな負担になります。そこで有効なのが量子化です。モデルの重みを低ビット化することで、メモリ使用量を抑えられます。一般的な Q4（4 ビット量子化）は、品質劣化をできるだけ抑えつつ、必要メモリを大きく削減できます。理論上は 1/4 になり、64 GB のモデルは 21 GB 前後で利用できます。
...
これで Claude Agent は Opus や Sonnet ではなく、指定したローカル LLM をバックエンドとして利用します。Claude Code はローカル LLM を利用して、アプリを実装します。

そこで GitHub のリポジトリを検索する iOS アプリの作成指示書を与えました。順調にアプリを製作していましたが、作成途中でローカル LLM の推論がクラッシュしました。
...
Mac は購入後にメモリ増設はできません。以前あった破格な 512 GB モデルはなくなり、また昨今の DRAM の価格高騰により、大容量メモリの Mac の購入は難しいです。そこで発想を変えて、LLM の実行を開発機の Mac から切り離します。
...
- LM Studio などは Windows / macOS / Linux をサポートするので、マシンを自由に選択できる
- 小中規模のローカル LLM なら、何とか個人でスペックを用意できる（要出典）
...
Chat は前述であった設定の Internet Hosted で、ローカル LLM サーバーのローカル IP アドレス（`192.168.100.38`など）を指定します。API Key などの初期値は未設定なので、適当な値でよいです。

![Chatにローカル LLM サーバーを追加する](./images/add-chat-internet.jpg)

Agent も同様に settings.json で localhost をローカル IP アドレスに書き換えます。これにより、先にクラッシュした Agent とローカル LMM は最後まで実装できました（詳細は後述を参照）。

## ローカル LLM に最適な自作 PC

私は自作 PC を趣味としており、今回いくつかのパターンで自作 PC を組みました。それらのベンチマークから、最適な構成を考えます。

LLM にもっとも影響するパーツは GPU です。本来は他パーツを固定して、GPU を入れ替えて計測するのが望ましいです。しかし、私の日常利用と自作ポリシーにより、構成はそれぞれ異なります。ご了承ください。なお、スペックはそれぞれミドルからミドルハイのレンジです。

### 検証機の比較

自作した検証機の構成の計測結果を表１にまとめました。ゲームなどで日常利用してるメイン PC、余ったパーツなどで組んだサブ PC たちです。今回のローカル LMM に向けて、私がこれならいいだろうと組んだ PC [^qiita-local-llm-pc] もあります。なお、それぞれ Windows 11 を採用して、パーツは中古、アウトレットやジャンクなどを多用しています。本当は DDR5 のメモリ（RAM）を多く載せたかったのですが、昨今の価格沸騰で断念しています。
...
難易度の高い要求でなければ、ローカル LLM は実装支援に活用できます。クラウドのトークン消費を抑えるための補助的な使い方が現実的です。

なお、実装中は検証機の GPU の消費電力とファン回転数がほぼ最大まで上がりました。LLM の計算負荷の高さを実感しました。

## まとめ

現在のローカル LLM は、開発対象を選べば十分に活用できます。LLM のモデルとハードウェアは継続的に進化しており、いずれは本格的な開発手段として定着していくでしょう。
