...
モデルのパラメータが 32B（320 億）の場合、単純計算で 1 パラメータあたり 16 bit（2 byte）なので約 64 GB のメモリが必要です。家庭用 PC には大きな負担です。そこで有効なのが量子化です。モデルの重みを低ビット化することで、メモリ使用量を抑えられます。一般的な Q4（４ビット量子化）は、品質劣化をできるだけ抑えつつ、メモリを大きく削減できます。理論上は 1/4 ですが、実際は KV キャッシュや実行時バッファなどがあるので、64 GB は約 21 GB になります。
...
Chat は前述であった手順の Internet Hosted で、ローカル LLM サーバーのローカル IP アドレスとポート番号（`192.168.100.38:1234`など）を指定します。API Key などの初期値は未設定なので、適当な値でよいです。
...
自作した検証機の構成の計測結果を表１にまとめました。ゲームなどで日常利用してるメイン PC、余剰パーツなどで組んだサブ PC たちです。また、今回のローカル LMM に向けて、私がこれならいいだろうと組んだ検証機 [^qiita-local-llm-pc] もあります。それぞれ Windows 11 を採用して、パーツは中古、アウトレットやジャンクなどを多用しています。本当は DDR5 のメモリ（RAM）を多く載せたかったのですが、昨今の価格沸騰で断念しています。
